...
class Factory(Base):
    ...
    environmental_friendliness: Mapped[int] = mapped_column(
        CheckConstraint("environmental_friendliness < 6", name="check_environmental_friendliness"),
        default=0
    )
    number_of_workers: Mapped[int] = mapped_column(default=0)
    workers_happiness: Mapped[int] = mapped_column(
        CheckConstraint("workers_happiness < 6", name="check_workers_happiness"),
        default=0
    )
    quality_of_equipment: Mapped[int] = mapped_column(
        CheckConstraint("quality_of_equipment < 6", name="check_quality_of_equipment"),
        default=0
    )
    production_per_hour: Mapped[int] = mapped_column(
    ...
    number_of_vehicles: Mapped[int] = mapped_column(default=0)

    moneys: Mapped[int] = mapped_column(default=10000)
    expenses_per_hour: Mapped[int] = mapped_column(
