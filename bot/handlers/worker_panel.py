...
SEND_THE_WORKERS_TO_REST_PRICE: int = 20
...
async def workers_msg_handler(message: Message) -> None:
    ...
    await message.answer_photo(
        ...
        caption=(
            ...
            f"• Отправить рабочих отдохнуть — '20$/чел' ({workers_count * SEND_THE_WORKERS_TO_REST_PRICE}) — `-1` к усталости"
        ),
...
async def teach_workers(call: CallbackQuery) -> None:
    ...
    if factory.quality_of_equipment <= (5 - PLUS_QUALITY_OF_PRODUCTS_WHEN_WORKERS_TEACHED) and factory.number_of_workers > 0:
        ...
        await call.answer(
            text=(
                 f"✅ Вы обучили рабочих!\n"
                 f"💵 Денег: {updated_factory.moneys}$\n"
                 f"😇 Качество техники: {updated_factory.workers_happiness:.1f}/5"
            ),
            show_alert=True
        )
    else:
        await call.answer(text="⚙️ У вас максимальный уровень качества техники или нет рабочих!", show_alert=True)
...
async def new_worker_count(call: CallbackQuery) -> None:
    ...
    await call.answer(
        text=(
             f"👷 Вы успешно наняли рабочих (+{count} чел.)!\n"
...
async def new_worker_count(call: CallbackQuery) -> None:
    ...
    await call.answer(
        text=(
             f"👷 Вы уволили рабочих (-{count} чел.)!\n"
             f"📊 Всего рабочих: {updated_factory.number_of_workers}\n"
...
@worker_panel_router.callback_query(F.data == "send_the_workers_to_rest")
async def send_the_workers_to_rest(call: CallbackQuery) -> None:
    factory = await get_factory_by_owner(call.from_user.id)
    ...
    price = factory.number_of_workers * SEND_THE_WORKERS_TO_REST_PRICE

    if factory.moneys < price:
        await call.answer(text="💸 У вас нехватает денег!", show_alert=True)
        return

    if factory.worker_fatigue <= 0:
        await call.answer(text="😇 Ваши рабочие не устали!", show_alert=True)
        return
    updated_factory = await update_factory(user_id=call.from_user.id, worker_fatigue=factory.worker_fatigue - 1)
    await call.answer(
        text=(
            "💤 Вы отправили рабочих отдохнуть!😇"
            f"☹️ Усталость рабочих: {updated_factory.worker_fatigue}"
        ),
        show_alert=True
    )
